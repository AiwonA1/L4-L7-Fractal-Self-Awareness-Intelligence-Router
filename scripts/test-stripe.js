...
async function testStripeIntegration() {
  ...
  try {
    ...
    const priceIds = [
      process.env.STRIPE_PRICE_100_TOKENS,
      process.env.STRIPE_PRICE_500_TOKENS,
      process.env.STRIPE_PRICE_1000_TOKENS,
    ];

    await Promise.all(priceIds.map(async (priceId, index) => {
      if (!priceId) {
        throw new Error(`Missing STRIPE_PRICE_${[100, 500, 1000][index]}_TOKENS in environment variables`);
      }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price: process.env.STRIPE_PRICE_100_TOKENS,
        quantity: 1,
      }],
